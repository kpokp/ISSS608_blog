...
### 2.1 Clarity
1. **Size of Bubbles**: 
    + Fix the size of the bubbles and add a legend to indicate the unit of measurement 'Annual Total Trade' and a note to bring it up for reader's attention.
...
3. **Y-Axis & X-Axis**: 
    ...
    + Both axes in rank chart will given more prominent tick marks for sufficient data inking.
4. **Footnotes**: 
    ...
    + We have removed redundant footnote and chose not to annotate highlights of changes in trade performance of partners over the year to keep the design neat. Dynamic tooltips with relevant information for each year are added to both charts.
...
2. **Colours and Labels**: 
    + Different colours are being used for each partners for the purpose of recognizing the trail of historical values in both charts as the visualisation animates.
    + Each bubble will retain the label with the name of the partner and total trade value of the year for easy reference.
