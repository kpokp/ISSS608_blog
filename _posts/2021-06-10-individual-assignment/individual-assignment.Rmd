...
trans_time <-   trans_final %>% 
                # Exclude locations with credit card posting timing issues to reduce spurious matches
...
We notice that the highest frequency matches need not coincide with the POI placement on the map. As such, we visually pick out the lat/long of each POI by checking the points of each POI on the interactive map above.
...
- Guy's Gyros: sw3tnev (uncertain - multiple points)
- Hallowed Grounds: sw3tnm1
- Hippokampos: sw3tjxs
- Jack's Magic Beans: sw3tnjb
- Kalami Kafenion: sw3tjt8
- Katerina's Cafe: sw3tnee (uncertain - multiple points)
- Kronos Mart: sw3tjmx
...
```{r coordinates, include = TRUE, warning = FALSE}
# view the POIs over the jpeg map
test <- filter(events_trans, id == "28")
map <- ggplot(test, aes(long, lat, text = paste("geohash:", geohash))) + 
...
POI_geohash <- tibble(location = c("Abila Airport","Abila Scrapyard","Abila Zacharo","Ahaggo Museum","Albert's Fine Clothing",
                      ...
                      geohash_likely = c("sw3thfv","sw3tjrh","sw3tjmp","sw3tnpd","sw3tjxs",
                                        "sw3tm2r","sw3tnek","sw3tnhm","sw3tnw2","sw3tn7s",
                                        "sw3tjxs","sw3tm9y","sw3tjq8","sw3tnsx","sw3tn4k",
...
Plot heatmap of events by id
```{r include = TRUE, warning = FALSE}
# Unknown places + Home + POI 

# Plot event timeline
event_timeline <- events_trans_2 %>%
                  
# Plot event data as heatmap
heatmap_cc <- ggplot(events_trans_2, aes(x= event_stop_time, y = name, color = location)) +
              theme_minimal() +
              geom_tile(colour="white") +
              labs(x = "Time", y = "Name") +
