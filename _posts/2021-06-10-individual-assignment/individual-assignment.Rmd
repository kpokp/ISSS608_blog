...
Time line chart paired with geospatial map was the most effective in communicating the anomalies and changes in an individual's activities. 
...
We spot some anomalies from the frequency of the spends that are worth investigating:
...
events_timeline <- ggplot(events_matched_long_highlight, 
  ...
  # Add label
  labs(title = "Locations by Car IDs", y = "Car ID", x = "Date", color = "Location Category") 
...
# Make gantt chart interactive
kronos_lag <- ggplotly(events_timeline_spend_KM, tooltip = c("text"))
kronos_lag
```
...
# Make gantt chart interactive
fraud_frydos <- ggplotly(events_timeline_spend_FA, tooltip = c("text"))
fraud_frydos
```
...
# Make gantt chart interactive
fixed_posting <- ggplotly(events_timeline_spend_cafe, tooltip = c("text"))
fixed_posting
```
...
# Make gantt chart interactive
dating <- ggplotly(events_timeline_2, tooltip = c("text"))
dating
...
events_timeline_5 <- ggplot() +
  ...
  # Add label
  labs(title = "Locations Visted By Gustav(9)", y = "Event Start Date", x = "Time", color = "Locations") 

# Make gantt chart interactive
bertrand <- ggplotly(events_timeline_5, tooltip = c("text"))
bertrand
```
...
events_timeline_4 <- ggplot() +
  ...
  # Add label
  labs(title = "Locations Visted By Lars (1) and  Minke (24)", y = "Event Start Date", x = "Time", color = "Locations") 
...
#### 5.4 Suspicious Locations

#### 5.4.1 Observe timeline of suspicious individuals to get hints on location
...
#### 5.4.2 Link timeline to map view to find suspicious locations
...
gps_path_1 <- gps_name_diff %>% 
  filter(id %in% c("13","15","21","24")) %>%
  filter(hour(timestamp) < 13 | hour(timestamp) > 11)
...
map <- ggplot(gps_path_1, aes(long, lat)) + 
  ...
  # Add label
  labs(title = "Visits to Suspicious Locations", color = "Location")
...
location_labels_sf <- st_as_sf(location_labels_all, coords = c("long", "lat"), crs = 4326) 
gps_path_selected <- gps_path %>% filter(id == c("29"))
...
```{r shared purchase & car pool, echo=FALSE}
dating
```
...
```{r credit card change, echo=FALSE}
bertrand
```
