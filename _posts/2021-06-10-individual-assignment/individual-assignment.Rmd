---
title: "Individual Assignment (WIP)"
description: |
  Student hard at work. Check back next time. 
draft: TRUE
preview: images/wip.png
author:
  - name: Kelly Koh
    url: https://www.linkedin.com/in/kellykkw/
    affiliation: School of Computing and Information Systems, Singapore Management University
    affiliation_url: https://scis.smu.edu.sg/
date: 06-10-2021
output:
  distill::distill_article:
    toc: true
    toc_depth: 3
    toc_float: true
    self_contained: false
---

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE)
knitr::opts_chunk$set(fig.retina = 3,
                      echo = FALSE,
                      eval = TRUE,
                      message = FALSE,
                      warning = FALSE)
```

## Background

### Vast Challenge 2021 - The Kronos Incident
In the roughly twenty years that Tethys-based GAStech has been operating a natural gas production site in the island country of Kronos, it has produced remarkable profits and developed strong relationships with the government of Kronos. However, GAStech has not been as successful in demonstrating environmental stewardship.

In January, 2014, the leaders of GAStech are celebrating their new-found fortune as a result of the initial public offering of their very successful company. In the midst of this celebration, several employees of GAStech go missing. An organization known as the Protectors of Kronos (POK) is suspected in the disappearance, but things may not be what they seem.

### Mini Challenge 2 Background
GAStech provides many of their employees with company cars for their personal and professional use, but unbeknownst to the employees, the cars are equipped with GPS tracking devices. We are given tracking data for the two weeks leading up to the disappearance, as well as credit card transactions and loyalty card usage data. From this data, we would identify anomalies and suspicious behaviors and identify which people use which credit and loyalty cards.

## Literature Review

### Overview on Previous Works
VAST Challenge 2021 made use of the same dataset that was used for VAST Challenge 2014. Of the 28 submissions for the Mini-Challenge 2 in 2014, many of the winning teams made use of D3.js, a JavaScript library to develop their propriety tools. Some of the teams explored novel approach such as using parallel coordinates plot, time rings and network graph. However, most common approaches can be summarized as:

- Timeline of employee’s location color coded by location category overlaid with spend
- Geospatial map of movement
- Space-time cube of movement

However, the results cannot be replicated as they are not using open-source software. 

### Our Approach
We would experiment with some of the visualizations above and seek a simpler representation using R, a open-source software, without compromising on the depth of the insights.



## Methodology and Visualisation 

### Understand the Spend Transactions

### Understand the Car Movement

### Unify View of Transactions and the Car Movement 


## R Packages and Datasets

### Load R Packages

```{r download packages, include = TRUE, warning = FALSE}
# The following R packages are used for respective parts of the workflow:
packages = c(
              # Loading data from csv files
               'readr',
              # Cleaning and manipulating data 
              'tidyverse', 'DT', 'fuzzyjoin',
              # Manipulate time and date variables
              'lubridate', 'clock',
              # Plot heat maps, gantt, parallel coordinates and scatter plot graphs
              'ggplot2', 'igraph', 'ggrepel' , 'ggally',
              # Plot interactive network graph
              'visNetwork',
              # Add interactivity to ggplot charts
              'plotly', 'gganimate',
              # Load maps & manipulate geo-spatial data
              'jpeg', 'grid', 'geohashTools', 'sf', 'tmap', 'patchwork', 'raster', 'rgdal', 'mapview')

for (p in packages) {
  if(!require(p, character.only = T)){
    install.packages(p)
  }
  library(p, character.only = T)
}
```

### Load Datasets

There are 4 datasets provided:

- **Credit card data**: transaction of each credit card with time stamp, price and location. 
    + 55 unique credit card numbers
    + 1490 cc transactions

```{r cc data, warning=FALSE, include=TRUE, paged.print=TRUE}
cc <- read_csv("data/cc_data.csv",
               col_types = cols(timestamp = col_datetime(format = "%m/%d/%Y  %H:%M"), 
                                location = col_character(), 
                                price = col_double(), 
                                last4ccnum = col_factor()))
glimpse(cc)
```

- **Loyalty card data**: transaction on loyalty card with date, price and location. 
    + 54 unique loyalty card numbers
    + 1392 loyalty card transactions

```{r loyalty data, include = TRUE, warning = FALSE}
loyalty <- read_csv("data/loyalty_data.csv",
               col_types = cols(timestamp = col_date(format = "%m/%d/%Y"), 
                                location = col_character(), 
                                price = col_double(), 
                                loyaltynum = col_factor()))
glimpse(loyalty)
```

- **GPS data**: latitude and longitude of each employee vehicle (ID) when in movement, including trucks which `id` is coded in '10X'
    + 685,169 records

```{r gps data, include = TRUE, warning = FALSE}
gps <- read_csv("data/gps.csv",
               col_types = cols(Timestamp = col_datetime(format = "%m/%d/%Y  %H:%M:%S"), 
                                id = col_factor(), 
                                lat = col_double(), 
                                long = col_double()))
glimpse(gps)
```

- **Car assignment data**: full names of employees and ID of cars assigned 

```{r car assignment data, include = TRUE, warning = FALSE}
cars <- read_csv("data/car-assignments.csv",
               col_types = cols(LastName = col_character(), 
                                FirstName = col_character(),
                                CarID = col_factor(), 
                                CurrentEmploymentType = col_character(),
                                CurrentEmploymentTitle = col_character()))
glimpse(cars)
```

## Data Cleaning and Transformation

### Credit and Loyalty Card transactions

#### Match names of locations in credit card and loyalty card transactions to check spelling

```{r POI names, warning=FALSE, include=TRUE}
cc_loc <- cc %>% distinct(location) %>% arrange(location) 

loyalty_loc <- loyalty %>% distinct(location) %>% arrange(location) 

location <- full_join(cc_loc, loyalty_loc, by = "location") %>% 
            arrange(location)
location
```

#### Replace the unrecognized characters in Katerina's Cafe location in cc & loyalty data

```{r update POI name, warning=FALSE, include=TRUE}
cc[grep("Katerina", cc$location),2] <- "Katerina's Cafe"
loyalty[grep("Katerina", loyalty$location),2] <- "Katerina's Cafe"
```


#### Check credit card transactions postings by locations by hour for patterns

```{r cc patterns, warning=FALSE, include=TRUE, paged.print=TRUE}
# Summarize cc data by hour
cc_hour <- cc %>% mutate(hour_cc = as.character.numeric_version(get_hour(timestamp))) %>% 
                  group_by(location,hour_cc) %>% 
                  count() %>%
                  rename("transactions" = n) %>%
                  ungroup()

# Plot cc hourly data as heatmap
heatmap_cc <- ggplot(cc_hour, aes(x= hour_cc, y = location, fill = transactions)) +
              theme_minimal() +
              geom_tile(colour="white") +
              labs(x = "Hour", y = "Location") +
              scale_fill_gradient(low = "#dfe1eb", high = "#261fb4") +
              labs(title = "Credit Card Spend by Hour",
                   x = "Hour", fill = "Transactions")

ggplotly(heatmap_cc)
```

We observed that some locations only have credit card transactions posted in 1 hour within the day e.g. 

- Bean There Done That and Brewed Awakenings, Jack's Magical Beans, Coffee Shack at 12PM
- Daily Dealz at 6AM. 

Another anomaly that we will investigate is the posting from Kronos Mart at 3AM.



#### Merge credit card transactions to loyalty transactions

- To merge using date, location and spend amount as loyalty card transactions do not have time
- To remove spurious matches, any match that only has 1 count will be removed

```{r merge cc & loyalty, include = TRUE, warning = FALSE}
# Create new date field for credit card data
cc <- cc %>% 
      mutate(datestamp = as.Date(timestamp))

# Join datasets on date, location and price 
transact <- full_join(cc,loyalty, by = c("datestamp" = "timestamp","location","price")) %>%             
            view()

# 1,087 paired trans - with possibility of spurious matches
transact %>% drop_na() %>% view()

# Find the unique pairs of credit card and loyalty card 
pairs <- transact %>% group_by(last4ccnum, loyaltynum) %>%
             count() %>%
             # Remove 6 spurious matches
             filter(n>1) %>%
             drop_na() %>%
             arrange(last4ccnum,loyaltynum) %>%
             ungroup() %>%
             view()
```

We will look into resolving certain data quality issues to enhance the pairing. As it is hard to visualize the interconnection between credit cards and loyalty cards in tabular form, we will use a network graph.  


#### Prepare cc & loyalty matching data for network graph format

```{r prep network graph, warning=FALSE, include=TRUE, paged.print=TRUE}
transact_match <- transact %>% drop_na()

last4ccnum <- transact_match %>%
  distinct(last4ccnum) %>%
  rename(label = last4ccnum) %>%
  mutate(group = 'cc')

loyaltynum <- transact_match %>%
  distinct(loyaltynum) %>%
  rename(label = loyaltynum) %>%
  mutate(group = 'loyalty')

nodes <- full_join(loyaltynum, last4ccnum, by = c("label","group"))

nodes <- nodes %>% rowid_to_column("id") %>% view()

per_route <- transact_match %>%  
  group_by(loyaltynum, last4ccnum) %>%
  summarise(weight = n()) %>% 
  ungroup()

edges <- per_route %>% 
  left_join(nodes, by = c("last4ccnum" = "label")) %>% 
  rename(from = id)

edges <- edges %>% 
  left_join(nodes, by = c("loyaltynum" = "label")) %>% 
  rename(to = id)

edges <- dplyr::select(edges, from, to, weight) %>% 
  filter(weight > 1) %>% 
  mutate(width = weight/5 + 1) %>% 
  view()
```

#### Plot network graph to show cc and loyalty cards relationship

```{r network graph of txn, warning=FALSE, include=TRUE, paged.print=TRUE}
visNetwork(nodes, edges, width = "100%", main = list(text = "Relationship Between Credit Card and Loyalty Cards",
 style = "font-family:arial;font-size:20px;")) %>% 
  visOptions(highlightNearest = TRUE, nodesIdSelection = list(enabled = TRUE,
                                                              selected = "74")) %>% 
  visGroups(groupname = "cc", color = "lightblue") %>%    
  visGroups(groupname = "loyalty", color = "orange") %>% 
  visLegend(width = 0.1, position = "right", main = list(text = "Card Types",
 style = "font-family:arial;font-size:15px;"))
```


- We found unexpected relationships: 1286 & L3288, L6267 - 6691 & 6899

#### Merge unmatched credit card transactions to loyalty transactions

- 326 loyalty card transactions and 424 credit card transactions were unpaired based on exact date, spend and location
- Based on the pairs of cc and loyalty numbers, we will match the remaining unmatched transactions based on date, location and price range, with loyalty price lower than credit card price.

```{r unmatched trans, warning=FALSE, include=TRUE, paged.print=TRUE}
# Create config table based on matched loyalty and credit card numbers 
config <- pairs %>% 
          filter(!(last4ccnum %in% c('1286') & loyaltynum %in% c('L3288'))) %>% 
          dplyr::select(last4ccnum, loyaltynum) %>%
          view()

# 1,066 transactions with exact match on date, loyalty number, price and location
exact_match <-  cc %>% 
                left_join(config, by = "last4ccnum") %>% 
                inner_join(loyalty, by = c("datestamp" = "timestamp",
                                           "loyaltynum",
                                           "location","price")) %>%
                view()

# 424 cc transactions exclude matches from exact price match
cc_unmatched <- cc %>% 
                anti_join(exact_match, by = c("datestamp", 
                                              "last4ccnum",
                                              "location","price")) %>%
                view()

# 326 loyalty transactions exclude matches from exact price match
loyalty_unmatched <- loyalty %>%
                     anti_join(exact_match, 
                               by = c("timestamp" = "datestamp",
                                      "loyaltynum","location","price")) %>%
                     view()

# Fuzzy match with loyalty price to be lower or equal to cc price
loyalty_cashback <- cc_unmatched %>% 
                    left_join(config, by = "last4ccnum") %>%
                    fuzzy_full_join(loyalty_unmatched, 
                    by = c("loyaltynum", "location",
                           "datestamp"="timestamp","price"="price"),
                    match_fun=list(`==`, `==`, `==`, `>=`)) %>%
                    view()

# 219 transactions match with price range with price difference in increments of $20.
loyalty_cashback <- loyalty_cashback %>%
                    drop_na() %>%
                    mutate(price_diff = price.x - price.y) %>% 
                    filter(round(price_diff,0) %in% c(20,40,60,80)) %>% 
                    view()
```

We discover that there are 219 transactions that have loyalty card price lower than credit card prices in a pattern. The lower amount registered on the loyalty card usually ranges from 20-80, in increments of 20s. This suggests that there might be a systemic program that led to this outcome e.g. cashback program.

#### Merge unmatched credit card transactions to loyalty transactions - round 2
- Based on the pairs of cc and loyalty numbers, we will match the remaining unmatched transactions based on location and price (and not date) to check for any delayed postings of credit card transactions.

```{r unmatched trans 2, warning=FALSE, include=TRUE, paged.print=TRUE}
# 205 cc transactions exclude matches from cashback match
cc_unmatched_2 <- anti_join(cc_unmatched, loyalty_cashback, 
                             by = c("datestamp", 
                                  "location" = "location.x",
                                  "price" = "price.x",
                                  "last4ccnum")) %>%
                view()

# 107 loyalty transactions exclude matches from cashback match
loyalty_unmatched_2 <- anti_join(loyalty_unmatched, loyalty_cashback, 
                                 by = c("timestamp" = "timestamp.y", 
                                        "loyaltynum" = "loyaltynum.y",
                                        "location" = "location.y",
                                        "price" = "price.y")) %>%
                     view()

cc_delay_post <- full_join(cc_unmatched_2, loyalty_unmatched_2, 
                          by = c("location", 
                                 "price")) %>%
                 view()

# 7 Kronos Mart posts credit card transactions posted 1 day late
# 15 transactions between 1286 & L3288
cc_delay_post_2 <- cc_delay_post %>% 
                  drop_na() %>%
                  mutate(diff_date = datestamp - timestamp.y) %>%
                  view()

```

We discover that Kronos Mart credit card transactions usually post 1 day late. And this match also teases out the unusual relationship between credit card 1286 and L3288. 


#### Merge unmatched credit card transactions to loyalty transactions - round 3
- Based on the pairs of cc and loyalty numbers, we will match the remaining unmatched transactions based on location and price range, whereby loyalty card price will be greater than credit card price

```{r unmatched trans 3, warning=FALSE, include=TRUE, paged.print=TRUE}
# 183 cc transactions exclude matches from delay posting
cc_unmatched_3 <- cc_delay_post %>% 
                  filter_all(any_vars(is.na(.))) %>% 
                  drop_na(last4ccnum) %>%
                  dplyr::select(timestamp.x, location, 
                              price, last4ccnum, 
                              datestamp) %>% 
                  view()

# 85 loyalty transactions exclude matches from delay posting
loyalty_unmatched_3 <- cc_delay_post %>% 
                      filter_all(any_vars(is.na(.))) %>% 
                      drop_na(loyaltynum) %>%
                      dplyr::select(location, price, 
                                   timestamp.y, loyaltynum) %>%
                      view()

# Match with loyalty price to be greater or equal to cc price 
cc_cashback <- left_join(cc_unmatched_3,config, 
                         by = "last4ccnum") %>% 
               fuzzy_full_join(loyalty_unmatched_3, 
               by = c("location",
                      "price",
                      "datestamp"="timestamp.y",
                      "loyaltynum"),
               match_fun=list(`==`, 
                              `==`, 
                              `<=`,
                              `==`)) %>%
               view()

```

No pattern was discovered from the match. We will disregard the matches. We will treat the remainder as situations whereby employees used one of the two cards.


#### Prepare final list of transaction records

```{r final trans, warning=FALSE, include=TRUE, paged.print=TRUE}
loyalty_cashback_final <- loyalty_cashback %>%
                          dplyr::select(timestamp.x,
                                       location.x, price.x, last4ccnum,
                                       datestamp, loyaltynum.x) %>%
                          rename(timestamp = timestamp.x, 
                          location = location.x,
                          price = price.x,
                          loyaltynum = loyaltynum.x)

cc_delay_post_final <- cc_delay_post_2 %>% 
                       dplyr::select(timestamp.x, 
                                    location, price, last4ccnum,
                                    datestamp, loyaltynum) %>%
                       rename(timestamp = timestamp.x)

trans_final <- exact_match %>%
               union_all(loyalty_cashback_final) %>%
               union_all(cc_delay_post_final) %>%
               view()

```

### GPS Logs and Map Layers

#### Set up map layers
```{r map layers, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Import MC2-tourist.tif created using QGIS
ap <- raster("data/Geospatial/MC2-tourist.tif")
ap

# Plotting raster layer using tmap to merge rgb bands
tm_shape(ap) +
tm_rgb(ap, r=1,g=2,b=3,
       alpha = NA,
       saturation = 1,
       interpolate = TRUE,
       max.value = 255)

# Use sf to import Vector GIS data files in ESRI shapefile format
abila_st <- st_read(dsn = "data/Geospatial",
                    layer = "Abila")
```

#### Converting aspatial data into simple feature data frame using st_as_sf
```{r echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
gps_sf <- st_as_sf(gps,
                   coords = c("long","lat"),
                            # EPSG 4326 which stands for wgs84 geo. coord sys
                            crs = 4326)
```

### Creating movement path from GPS points 
```{r echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
gps_path <- gps_sf %>%
            group_by(id) %>%
            # Due to peculiar nature of group by, we need to do an action such as summarize
            summarize(m = mean(Timestamp),
                      do_union = FALSE) %>%
            st_cast("LINESTRING") %>%
            ungroup()
            
gps_path
# Have to slice the paths by days, or time period to derive meaningful paths
```

#### Plotting the gps paths by employees to detect gps anomalies
```{r plot gps, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
gps_path_selected <- gps_path %>% view()

tmap_mode("view")

tm_shape(ap) +
  tm_rgb(ap, r=1,g=2,b=3,
         alpha = NA,
         saturation = 1,
         interpolate = TRUE,
         max.value = 255) +
  tm_shape(gps_path_selected) +
  tm_lines() +
  tm_facets(by = "id")
```

We noticed that Car ID 28 and 9 have gps data issues:

- Car ID 28's gps coordinates are systematically misaligned, which we can shift back using GAStech as the anchoring position and averaging out the coordinates
- Car ID 9's gps records are spotty, journeys did not end and start in the same place

```{r faulty gps, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
gps_path_selected <- gps_path %>% filter(id %in% c("28","9"))

tmap_mode("view")

tm_shape(ap) +
  tm_rgb(ap, r=1,g=2,b=3,
         alpha = NA,
         saturation = 1,
         interpolate = TRUE,
         max.value = 255) +
  tm_shape(gps_path_selected) +
  tm_lines() +
  tm_facets(by = "id")
```


#### Resolve data issues in GPS log
```{r gps clean, include = TRUE, warning = FALSE}

gps$Timestamp <- as.POSIXct(gps$Timestamp, 
                                 format = "%m/%d/%Y  %H:%M:%S")

# Group timestamp in intervals of 15 seconds and select the mean coordinates
gps_28 <- gps %>%   
            filter(id == "28") %>% 
            mutate(Timestamp = round_date(Timestamp, "15 sec")) %>%
            mutate(lat = round(lat - 00.004,3) ) %>% 
            mutate(long = round(long + 00.004,3) ) %>%
            group_by(Timestamp) %>%
            summarise(lat = mean(lat, na.rm = FALSE), long = mean(long, na.rm = FALSE)) %>% 
            mutate(id = "28") %>%
            ungroup() %>% view()


gps_sf_28 <- st_as_sf(gps_28,
                   coords = c("long","lat"),
                            # EPSG 4326 which stands for wgs84 geo. coord sys
                            crs = 4326)

gps_path_28 <- gps_sf_28 %>%
            group_by(id) %>%
            # Due to peculiar nature of group by, we need to do an action such as summarize
            summarize(m = mean(Timestamp),
                      do_union = FALSE) %>%
            st_cast("LINESTRING") %>%
            ungroup()


gps_path_selected_28 <- gps_path_28

tmap_mode("view")

tm_shape(ap) +
  tm_rgb(ap, r=1,g=2,b=3,
         alpha = NA,
         saturation = 1,
         interpolate = TRUE,
         max.value = 255) +
  tm_shape(gps_path_selected_28) +
  tm_lines() +
  tm_facets(by = "id")

```

#### Detect events from GPS log

- To define all stops between GPS log records that spans above 5 minutes as events
- Each event will have a time span, a location, and a person
- Identify home of employee as the location with the location with the longest span

```{r gps prep, include = TRUE, warning = FALSE}
# Replace gps records of Car ID 28
gps_28 <- gps_28 %>%
          dplyr::select(Timestamp, id, lat, long)

gps_clean <- gps %>% 
              filter(!id == "28") %>%
              rbind(gps_28) %>% view()

# Merge car assignments to gps data & sort table by id & timestamp
gps_name <- left_join(gps_clean,cars, by = c("id" = "CarID")) %>%
            unite("name", FirstName, LastName, sep = " ") %>%
            arrange(id,Timestamp)

# Change timestamp format in gps
gps_name$Timestamp <- as.POSIXct(gps_name$Timestamp, 
                                 format = "%m/%d/%Y  %H:%M:%S")

# Find difference between timestamp and id
gps_name_temp <- dplyr::select(gps_name,Timestamp)
time_diff <- tail(gps_name_temp,-1) - head(gps_name_temp,-1)

# Join & Tag Start and Stop for Intervals > 5 min
gps_name_diff <- bind_cols(time_diff$Timestamp,tail(gps_name,-1)) %>%
    rename(diff=...1) %>%
    group_by(id) %>%
  
    # If interval btw timestamps is more than 5 mins, tag as an event
    mutate(event_stop = ifelse(diff/60 > 5,'stop',
                          ifelse(diff/60 < 0,'stop',NA))) %>%
    mutate(event_start = lead(event_stop)) %>% 
  
    # If interval between timestamps is more than 5 hours, tag as home to help identify home locations later
    mutate(home = ifelse(diff/60/60 > 5,'home',NA)) %>%
    mutate(diff = seconds_to_period(diff)) %>%
    mutate(event_start = ifelse(event_start == 'stop','start',NA)) %>%
  
    # Encode lat long into geohash level 7    
    mutate(geohash = gh_encode(latitude = lat, 
                               longitude = long, 
                               precision = 7L)) %>%
    ungroup()
```

#### Create table that captures all events 
- These locations would be later identified as home, places of interest, or suspicious locations. 

```{r events, include = TRUE, warning = FALSE}
# 3068 events that span more than 5 minutes
events <- gps_name_diff %>% 
          filter(event_stop == "stop") %>%
          mutate(event_start_time = Timestamp - diff) %>%
          rename(event_stop_time = Timestamp) %>% 
          mutate(event_date = as_date(event_start_time)) %>% 
          dplyr::select(event_stop, home,event_date, event_start_time, event_stop_time,
                        diff, lat, long, geohash, id, name) %>%
          filter(diff > 0) %>%
          view()

glimpse(events)

```

#### Create a factor to capture the sequence to feature the employees
```{r id level, include = TRUE, warning = FALSE}
id_level <- gps_name %>% 
            distinct(id) %>%
            mutate(id = as.numeric(id)) %>%
            arrange(id) %>%
            mutate(id = as.factor(id)) %>%
            view()
```

#### Identify home locations of employees
```{r home events, include = TRUE, warning = FALSE}
# Identify homes or place of accommodation of employees - 501 events
home <-   events %>% 
          filter(home == "home") %>%
          view()

# view the home locations over the jpeg map
map <- ggplot(home, aes(long, lat, text = paste("geohash:", geohash))) + 
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +

  # Add points
    geom_point(aes(color=id, alpha = 0.8))

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)
layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.7,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))

```


From the interactive map, we observe that some of the employees stay together e.g. employees with Car ID 30, 22 and 23. 

Furthermore, due to GPS issues, employee with Car ID 28 has home locations spread around a concentrated area, while employee with Car ID 9 spreads out over a wider area. 

We also noticed that we picked up GASTech as a location especially for truck drivers, who parks their vehicles at the company at the end of their work day.

#### Identify likely geohash of employees' homes locations 
```{r home config, include = TRUE, warning = FALSE}
location_labels_home <-  home %>% 
                mutate(location = ifelse(geohash == "sw3tn4k", "GASTech",
                ifelse(geohash == "sw3tnjw", "Home 1",
                ifelse(geohash == "sw3tm9n", "Home 2",
                ifelse(geohash == "sw3tm9k", "Home 3",
                ifelse(geohash == "sw3tjzz", "Home 4",
                ifelse(geohash == "sw3tnjg","Home 5",
                ifelse(geohash == "sw3tnkr","Home 6/25/29",
                ifelse(geohash == "sw3tmbc","Home 7",
                ifelse(geohash == "sw3tnmc","Home 8",
                ifelse(geohash %in% c("sw3tm2r","sw3tm8g"),"Home 9",
                ifelse(geohash == "sw3tjzd","Home 10",
                ifelse(geohash == "sw3tm8v","Home 11",
                ifelse(geohash %in% c("sw3tntg","sw3tntf"),"Home 12",
                ifelse(geohash == "sw3tnsx","Home 13/15/16/21",
                ifelse(geohash %in% c("sw3tnt0"),"Home 14/18",
                ifelse(geohash == "sw3tnsf","Home 17/24/33",
                ifelse(geohash == "sw3tm9e","Home 19",
                ifelse(geohash == "sw3tnts","Home 20",
                ifelse(geohash %in% c("sw3tnsh"),"Home 22/23/30",
                ifelse(geohash %in% c("sw3tnqb","sw3tnnz"),"Home 26",
                ifelse(geohash == "sw3tnq0","Home 27",
                ifelse(geohash %in% c("sw3tnnj","sw3tnnh"),"Home 28",
                ifelse(geohash == "sw3tnw2","Chostus Hotel",
                ifelse(geohash == "sw3tnn9","Home 32",
                ifelse(geohash == "sw3tntv","Home 34",
                ifelse(geohash == "sw3tnp3","Home 35", NA))))))))))))))))))))))))))) %>%
                drop_na() %>%
                group_by(location) %>%
                slice_head() %>%
                ungroup() %>% 
                mutate(category = ifelse(location == "GASTech", "Company", 
                                  ifelse(location == "Chostus Hotel", "Leisure","Home"))) %>%
                dplyr::select("lat","long","geohash","location","category") %>%
                view()
```

#### Identify anomalies from home stays
```{r home matched, include = TRUE, warning = FALSE}
# Match home events to home config table based on 150m radius and no duplicate was introduced - 494 events matched
# Unknown locations will be investigated
home_matched <- home %>% 
                geo_left_join(location_labels_home, by = c("lat", "long"), max_dist = 0.15, distance_col = "distance", unit = c("km")) %>% 
                # Assume employee with car ID 9 to be home
                mutate(location = ifelse(id == 9, "Home 9",location)) %>%
                view()

# Custome theme for clean Gantt Chart
theme_gantt <- function(base_size=11, base_family="Source Sans Pro Light") {
  ret <- theme_bw(base_size, base_family) %+replace%
    theme(panel.background = element_rect(fill="#ffffff", colour=NA),
          axis.title.x=element_text(vjust=-0.2), axis.title.y=element_text(vjust=1.5),
          title=element_text(vjust=1.2, family="Source Sans Pro Semibold"),
          panel.border = element_blank(), axis.line=element_blank(),
          panel.grid.minor=element_blank(),
          panel.grid.major.y = element_blank(),
          panel.grid.major.x = element_line(size=0.5, colour="grey80"),
          axis.ticks=element_blank(),
          legend.position="bottom", 
          axis.title=element_text(size=rel(0.8), family="Source Sans Pro Semibold"),
          strip.text=element_text(size=rel(1), family="Source Sans Pro Semibold"),
          strip.background=element_rect(fill="#ffffff", colour=NA),
          panel.spacing.y=unit(1.5, "lines"),
          legend.key = element_blank())
  
  ret
  }

# Convert data to long format for ggplot
# home_matched_long <- home_matched %>%
                #dplyr::select(id, event_start_time, event_stop_time, location) %>%
                #gather(date.type, id.date, -c(location, id)) %>%
                #arrange(date.type, id.date) %>%
                #mutate(id = factor(id, levels=rev(unique(id)))) %>% view()
# Build plot
#home_timeline <- ggplot(home_matched_long, aes(x=id, y=id.date, colour=location)) + 
  #geom_line(size=1) + 
  #guides(colour=guide_legend(title=NULL)) +
  #labs(x=NULL, y=NULL) + coord_flip() +
  #scale_y_date(date_breaks="1 day", labels=date_format("%b ‘%y")) +
  #theme_gantt() + theme(axis.text.x=element_text(angle=45, hjust=1)) +
  #geom_point()

# Make gantt chart interactive
#ggplotly(home_timeline)
  
# Plot timeline data as gantt chart
home_timeline <- qplot(ymin = event_start_time,
                    ymax = event_stop_time, 
                    x = id,
                    color = location,
                    geom = "linerange",
                    data = home_matched,
                    size = I(1)) +
              coord_flip() +
              ggtitle("Timeline") +
              scale_y_datetime(date_breaks="1 day") + 
              theme_gantt() + theme(axis.text.x=element_text(angle=45, hjust=1))

# Make gantt chart interactive
ggplotly(home_timeline)
  
```

From the timeline above, we observe some activities that stand out that we will investigate further, 
- ID 1 was not home for 2 nights (6 Jan and 8 Jan)
- ID 16 was staying overnight in Aliba starting 7 Jan 
- ID 21 visited 2 homes from 10 Jan and visited an unknown location subsequently. Home location was also not detected on night of 16 Jan.
- ID 31 was only in Aliba for 4day-3nights (16 to 20 Jan) in the Chostus hotel
- Selected trucks were only parked at Aliba from 6 Jan to 16 Jan, and they are parked at the GASTech building


#### Find out the lat long of credit card events based on time frame
- If the transaction time is within the time span of an event, it is assigned to that event

```{r event and transactions, include = TRUE, warning = FALSE}
# Select events that are not staying at home by excluding home - 2567 events
events_cc <- events %>% 
          filter(is.na(home)) %>%
          view()

# Select distinct timestamp with each location from transactions
trans_time <-   trans_final %>% 
                dplyr::select(location, timestamp) %>% 
                distinct(location, timestamp) 
               
# Fuzzy match of events and transactions by time windows
events_trans <- events_cc %>% 
                fuzzy_left_join(trans_time, 
                by = c("event_start_time" = "timestamp", "event_stop_time" = "timestamp"),
                match_fun = list(`<`,`>`)) %>%
                view()   

# Prepare the matched data for visualization. 
# We would not pick the top matches as some locations are less visited.
POI <- events_trans %>% dplyr::select(!home) %>% 
                 drop_na() %>%  
  
                 # Round the lat long to reduce effect of spotty gps
                 mutate(lat = round(lat,3), long = round(long,3)) %>% 
                 dplyr::select(location, lat, long, geohash) %>% 
                 group_by(location, lat, long, geohash) %>% 
                 summarise(n = n()) %>%
                 ungroup() %>% 
                 arrange(location, desc(n)) %>% 
                 group_by(location) %>%
                 ungroup() %>% 
                 view()

```

####  Plot the events that coincide with transactions to determine the lat long of POIs
```{r POI coordinates, include = TRUE, warning = FALSE}
# view the POIs over the jpeg map
map <- ggplot(POI, aes(long, lat, text = paste("geohash:", geohash))) + 
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +

  # Add points
    geom_point(aes(size = n, color=location, alpha = 0.8))

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)
layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.7,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))
```


We double click on the location names on the interactive map to see where did the credit card and gps locations coincide. We notice that the highest frequency matches need not coincide with the POI placement on the map. As such, we visually pick out the geohash of each POI based on the the description on the map, coupled with the highest occurrence. 

Most likely geohashes identified per location:

- Abila Airport: sw3thfv
- Abila Scrapyard: sw3tjrh
- Abila Zacharo: sw3tjmp
- Ahaggo Museum: sw3tnpd
- Albert's Fine Clothing: sw3tjx7
- Bean There Done That: sw3tm2r
- Brew've Been Served: sw3tnek 
- Carlyle Chemical Inc.: sw3tnhm
- Chostus Hotel: sw3tnw2
- Coffee Cameleon: sw3tn7s
- Desafio Golf Course: sw3tm9y
- Frank's Fuel: sw3tjq8
- Frydos Autosupply n' More: sw3tnes 
- GAStech: sw3tn4k
- Gelatogalore: sw3tjsm
- General Grocer: sw3tjse
- Guy's Gyros: sw3tnev 
- Hallowed Grounds: sw3tnm1
- Hippokampos: sw3tjxs
- Jack's Magic Beans: sw3tnjb
- Kalami Kafenion: sw3tjt8
- Katerina's Cafe: sw3tnee 
- Kronos Mart: sw3tjmx
- Kronos Pipe and Irrigation: sw3tjj6
- Maximum Iron and Steel: sw3tjm2
- Nationwide Refinery: sw3tnk1 
- Octavio's Office Supplies: sw3tjsm
- Ouzeri Elian: sw3tjgn
- Roberts and Sons: sw3tjt0
- Shoppers' Delight: sw3tjgh
- Stewart and Sons Fabrication: sw3tng3
- U-Pump: sw3tjvy
- Kronos Capitol: sw3tj7n


```{r id 28, include = TRUE, warning = FALSE}
# view the POIs over the jpeg map
gps_28 <- filter(events_trans, id == "28")
map <- ggplot(gps_28, aes(long, lat, text = paste("geohash:", geohash))) + 
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +

  # Add points
    geom_point(aes(color=location, alpha = 0.8))

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)
layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.7,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))
```

#### Create a config table that holds the likely geohashes of each POI
```{r POI config, include = TRUE, warning = FALSE}
# Create a config table of the POI with its most likely geohash
POI_geohash <- tibble(location = c("Abila Airport","Abila Scrapyard","Abila Zacharo","Ahaggo Museum","Albert's Fine Clothing",
                                   "Bean There Done That","Brew've Been Served","Carlyle Chemical Inc.","Chostus Hotel","Coffee Cameleon",
                                   "Desafio Golf Course","Frank's Fuel","Frydos Autosupply n' More","GAStech",
                                   "Gelatogalore","General Grocer","Guy's Gyros","Hallowed Grounds","Hippokampos",
                                   "Jack's Magic Beans","Kalami Kafenion","Katerina's Cafe","Kronos Mart","Kronos Pipe and Irrigation",
                                   "Maximum Iron and Steel","Nationwide Refinery","Octavio's Office Supplies","Ouzeri Elian","Roberts and Sons",
                                   "Shoppers' Delight","Stewart and Sons Fabrication","U-Pump","Kronos Capitol"),
                      geohash_likely = c("sw3thfv","sw3tjrh","sw3tjmp","sw3tnpd","sw3tjx7",
                                        "sw3tm2r","sw3tnek","sw3tnhm","sw3tnw2","sw3tn7s",
                                        "sw3tm9y","sw3tjq8","sw3tnes","sw3tn4k",
                                        "sw3tjsm","sw3tjse","sw3tnev","sw3tnm1","sw3tjxs",
                                        "sw3tnjb","sw3tjt8","sw3tnee","sw3tjmx","sw3tjj6",
                                        "sw3tjm2","sw3tnk1","sw3tjsm","sw3tjgn","sw3tjt0",
                                        "sw3tjgh","sw3tng3","sw3tjvy","sw3tj7n")) %>% 
                view()

```

#### Match events to POI locations based on likely geohash to enhance list with lat long infor
```{r likely geohash, include = TRUE, warning = FALSE}
events_trans_2 <- events_trans %>% dplyr::select(!home) %>% 
                  drop_na() %>% 
                  left_join(POI_geohash, by = "location") %>%   
                  filter(geohash == geohash_likely) %>% 
                  dplyr::select(!timestamp) %>%
                  distinct() %>%
                  view()
```

#### Assign categories to add semantics to points-of-interest (POI)
```{r POI category, warning=FALSE, include=TRUE, paged.print=TRUE}
food <- c("Abila Zacharo","Bean There Done That","Brew've Been Served",
          "Brewed Awakenings","Coffee Cameleon","Coffee Shack","Gelatogalore",
          "Guy's Gyros", "Hallowed Grounds","Hippokampos","Jack's Magical Beans", 
          "Kalami Kafenion", "Katerina's Cafe","Ouzeri Elian")
retail <- c("Albert's Fine Clothing","Daily Dealz","General Grocer","Kronos Mart",
            "Roberts and Sons", "Shoppers' Delight")
gas <- c("Frank's Fuel","U-Pump")
leisure <- c("Abila Airport","Ahaggo Museum","Chostus Hotel","Desafio Golf Course","Kronos Capitol")
company <- c("Abila Scrapyard","Carlyle Chemical Inc.","Frydos Autosupply n' More",
             "Kronos Pipe and Irrigation","Maximum Iron and Steel","GASTech",
             "Nationwide Refinery","Octavio's Office Supplies","Stewart and Sons Fabrication")

events_trans_2 <- events_trans_2 %>% mutate(category = 
                    ifelse(location %in% food, "Food",
                    ifelse(location %in% retail, "Retail",
                    ifelse(location %in% gas, "Gas",
                    ifelse(location %in% leisure, "Leisure",
                    ifelse(location %in% company, "Company", NA)))))) %>% view()

location_labels_POI <- events_trans_2 %>%  drop_na() %>%
                group_by(location) %>%
                slice_head() %>%
                ungroup() %>% 
                dplyr::select("lat","long","geohash","location","category") %>% 
  
                # Identified locations based on windows of other food location + label on map 
                # as the cc timing did not match gps due to cc posting issues or non-spend locations
                add_row(lat = 36.068, long = 24.873, geohash = "sw3tnjb", location = "Jack's Magical Beans", category = "Food") %>%
                add_row(lat = 36.066, long = 24.850, geohash = "sw3tjmx", location = "Kronos Mart", category = "Retail") %>% 
                add_row(lat = 36.082, long = 24.851, geohash = "sw3tm2r", location = "Bean There Done That", category = "Food") %>%
                add_row(lat = 36.073, long = 24.864, geohash = "sw3tjycr", location = "Brewed Awakenings", category = "Food") %>%
                add_row(lat = 36.048, long = 24.880, geohash = "sw3tn4k", location = "GASTech", category = "Company") %>%
                ungroup() %>% view()


# Check that the positions are correctly tagged on the map
map <- ggplot(location_labels_POI, aes(long, lat, text = paste("geohash:", geohash))) + 
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +

  # Add points
    geom_point(aes(color= category, alpha = 0.8)) +
    geom_text(aes(label =location), size=2)

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)

layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.3,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))

```

#### Match the rest of the 2567 events + 7 unknown events with POIs and home locations
```{r POI matched, include = TRUE, warning = FALSE}

# Create a config table with both POI and home locations
location_labels <- location_labels_POI %>% rbind(location_labels_home) %>%
                   group_by(location) %>%
                   slice_head() %>%
                   ungroup()

# Add the unknown locations from home events match to the rest of the events
unknown_home_location <- home_matched %>% filter(is.na(location)) %>%
                         dplyr::select(event_stop,home,event_date, event_start_time,event_stop_time,diff,lat.x,long.x,geohash.x,id,name) %>%
                         rename(lat = lat.x, long = long.x, geohash = geohash.x)

# Match rest of events to POI config table based on 100m radius and pick the location that is closest
# Unknown locations will be investigated
POI_matched <-  events_cc %>% 
                rbind(unknown_home_location) %>%
                geo_left_join(location_labels, by = c("lat", "long"), max_dist = 0.1, distance_col = "distance", unit = c("km")) %>% 
                group_by(id, event_stop_time) %>%
                arrange(desc(distance)) %>%
                slice_head() %>%
                view()
  
# Plot timeline data as gantt chart
POI_timeline <- qplot(ymin = event_start_time,
                    ymax = event_stop_time, 
                    x = id,
                    color = location,
                    geom = "linerange",
                    data = POI_matched,
                    size = I(1)) +
              coord_flip() +
              ggtitle("Timeline") +
              scale_y_datetime(date_breaks="1 day") + 
              theme_gantt() + theme(axis.text.x=element_text(angle=45, hjust=1))

# Make gantt chart interactive
ggplotly(POI_timeline)

# TRY PLOTLY GANTT CHART so that hover tooltip can be used

```

#### Identify unknown events on the map
```{r include = TRUE, warning = FALSE}
# Isolate the unknown events
unknown_location <- POI_matched %>% filter(is.na(location)) %>%
                         dplyr::select(event_stop,home,event_start_time,event_stop_time,diff,lat.x,long.x,geohash.x,id,name) %>%
                         rename(lat = lat.x, long = long.x, geohash = geohash.x)

map <- ggplot(unknown_location, aes(long, lat, text = paste("geohash:", geohash))) + 
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +

  # Add points
    geom_point(aes(color=id, alpha = 0.8)) +
    geom_text(aes(label =id), size=1)

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)

layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.7,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))
```

We observe some patterns from the map above:
- Car ID 9's spotty gps cause events to be identified when there is none. We will ignore the events until we fix the journeys of Car ID 9.
- Even though we have tried to fix Car ID 28's gps issue, it shows that the lat long of the events are not within the 100m of the POI. Visually, they seem innocuous, being close to Jack's Magic Beans, Brew've Been Served and Kronos Capital. We will do another round of match to tag these events. 
- Excluding events from 9 and 28, and locations that are near POI, there are 5 locations that cannot be explained. They are visited by Car ID 13, 15, 21 and 24

#### Do a match of all events against the 3 catgeories of locations - home, POI and suspicious
```{r include = TRUE, warning = FALSE}
# Tag suspicious locations
location_labels <- location_labels %>% 
  add_row(lat = 36.05293, long = 24.84937, geohash = "sw3tj7n", location = "Kronos Capitol", category = "Leisure") %>%
  add_row(lat = 36.05022, long = 24.90047, geohash = "sw3tnde", location = "Suspicious Location 1", category = "Suspicious") %>%
  add_row(lat = 36.06318, long = 24.90019, geohash = "sw3tnt5", location = "Suspicious Location 2", category = "Suspicious") %>%
  add_row(lat = 36.05895, long = 24.89285, geohash = "sw3tnkq", location = "Suspicious Location 3", category = "Suspicious") %>%
  add_row(lat = 36.08062, long = 24.84688, geohash = "sw3tm2k", location = "Suspicious Location 4", category = "Suspicious") %>%
  add_row(lat = 36.06946, long = 24.84157, geohash = "sw3tjq1", location = "Suspicious Location 5", category = "Suspicious") %>%
  view()

# Match all 3068 events 
# 13 events unmatched, 12 of them from Car ID 9 and 1 from Car ID 28.
events_matched <-  events_cc %>% 
                rbind(unknown_home_location) %>%
                geo_left_join(location_labels, by = c("lat", "long"), max_dist = 0.1, distance_col = "distance", unit = c("km")) %>% 
                group_by(id, event_stop_time) %>%
                arrange(desc(distance)) %>%
                slice_head() %>%
                rbind(filter(home_matched, !is.na(location)))%>% 
                view()
```

#### Match Car IDs with credit card and loyalty card IDs
```{r include = TRUE, warning = FALSE}
# Fuzzy match with credit card and loyalty card transactions

loyalty_unmatched_3 <- loyalty_unmatched_3 %>% rename(datestamp = timestamp.y)
cc_unmatched_3 <- cc_unmatched_3 %>% rename(timestamp = timestamp.x)
trans_all <- trans_final %>% 
             bind_rows(loyalty_unmatched_3) %>% 
             bind_rows(cc_unmatched_3) %>% view()


car_spend_match <- trans_final %>% 
                fuzzy_left_join(events_matched, 
                by = c("timestamp" = "event_start_time", "timestamp" = "event_stop_time", "location" = "location"),
                match_fun = list(`>`,`<`,`==`)) %>%
                view()   

car_spend_match_2 <- car_spend_match %>% dplyr::select(id, name, last4ccnum, loyaltynum) %>%
                    group_by(id, name, last4ccnum, loyaltynum) %>%
                    summarise(n = n()) %>%
                    ungroup() %>%
                    drop_na() %>%
                    group_by(id) %>%
                    arrange(desc(n)) %>%
                    slice_head() %>%
                    ungroup() %>%
                    view()

car_spend_match_3 <- events_matched %>% left_join(car_spend_match_2, by = "id") %>%
                     fuzzy_left_join(trans_all, 
                                  by = c("event_start_time" = "timestamp","event_stop_time" =  "timestamp", 
                                         "location" = "location", "last4ccnum" = "last4ccnum"),
                                  match_fun = list(`<`,`>`,`==`,`==`)) %>%
                     view()   

#car_spend_match_4 <- car_spend_match_3 %>% filter(is.na(price)) %>% 
                     #(trans_all, 
                                  #by = c("event_date" = "datestamp",
                                         #"location.x" = "location", "last4ccnum.x" = "last4ccnum")) %>%
                     #view()   


car_spend_match_4 <-  car_spend_match_3 %>%
                      dplyr::select(event_date, event_start_time, event_stop_time, diff, lat.x, long.x, 
                                  geohash.x, location.x, id, name.x, last4ccnum.x, loyaltynum.x, 
                                  timestamp, datestamp, price) %>% 
                      rename(lat = lat.x, long = long.x, geohash = geohash.x, location = location.x,  
                            name = name.x, last4ccnum= last4ccnum.x, loyaltynum = loyaltynum.x)

# Plot timeline data as gantt chart
events_timeline <- qplot(ymin = event_start_time,
                    ymax = event_stop_time, 
                    x = id,
                    color = location,
                    geom = "linerange",
                    data = car_spend_match_4,
                    size = I(1)) +
              coord_flip() +
              ggtitle("Timeline") +
              scale_y_datetime(date_breaks="1 day") + 
              theme_gantt() + theme(axis.text.x=element_text(angle=45, hjust=1))

# Make gantt chart interactive
ggplotly(events_timeline)
```

#### Prep data for timeline linking to map
```{r include = TRUE, warning = FALSE}


map <- ggplot(location_labels, aes(long, lat, text = paste("geohash:", geohash))) + 
      
    
    theme(panel.border = element_blank(),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(),
    axis.line = element_line(colour = "grey")) +
  
  # Add paths
    geom_line(gps_path, aes())

  # Add points
    geom_point(aes(color=category, alpha = 0.3)) +
    geom_text(aes(label =location), size=2)

# Use ggplotly to make it interactive so we can pick the right geohash visually
pp <- ggplotly(map)
layout(pp, images = list(
  list(
    source = "https://raw.githubusercontent.com/kpokp/ISSS608_blog/main/_posts/2021-06-10-individual-assignment/images/MC2-tourist.jpg",
    opacity = 0.3,
    layer = "below",
    xref = "x",
    yref = "y",
    sizing="stretch",
    x= 24.8244,
    y= 36.0952,
    sizex= 0.0852,
    sizey= 0.05
  )
))
```

#### Prep data for timeline linking to map
```{r include = TRUE, warning = FALSE}

location_labels_sf <- st_as_sf(location_labels, coords = c("long", "lat"), crs = 4326) 
gps_path_selected <- gps_path %>% filter(id == c("1"))

tmap_mode("view")

tm_shape(ap) +
  tm_rgb(ap, r=1,g=2,b=3,
         alpha = NA,
         saturation = 1,
         interpolate = TRUE,
         max.value = 255) +
tm_shape(gps_path_selected) +
  tm_lines() +
tm_shape(location_labels_sf) +
  tm_squares(col = "category",
          id = "location", 
          popup.vars = "Location Type:" =="category",
          size = 0.1)
```












## Insights and Conlusions

### Question 1 
Using just the credit and loyalty card data, identify the most popular locations, and when they are popular. What anomalies do you see? What corrections would you recommend to correct these anomalies? Please limit your answer to 8 images and 300 words. 

::: l-body
|No| Commentary          | Visual                                       |
|--|---------------------|----------------------------------------------|
|1| Popular locations | ![](./images/Q3_1.png){#id .class width=100%}|
|2| Anomaly - Credit card postings at fixed hour | ![](./images/Q3_2.png){#id .class width=100%}|
|3| Anomaly - Kronos Mart Postings are 1 day late | ![](./images/Q3_3.png){#id .class width=100%}|
|4| Anomaly - Cashbacks in Multiples of $20 | ![](./images/Q3_4.png){#id .class width=100%}|
|5| text| ![](./images/Q3_5.png){#id .class width=100%}|
|6| text| ![](./images/Q3_6.png){#id .class width=100%}|
|7| text| ![](./images/Q3_7.png){#id .class width=100%}|
|8| text| ![](./images/Q3_8.png){#id .class width=100%}|
:::


#### Q1 Most popular locations
The most popular locations belong to the Food categories, such as Katerina's Cafe, Brew've Been Served and Hippokampos. Brew've Been Serve closes on weekends, while Katerina's Cafe peaks on Saturday.
```{r popular locations, echo=FALSE, warning=FALSE, include=TRUE}
# Aggregate transactions by day
trans_all_day <- trans_all %>%
                 left_join(location_labels_POI, by = "location") %>%
                 dplyr::select(category, datestamp, location) %>% 
                 group_by(category, datestamp, location) %>%
                 summarise(transactions = n()) %>%
                 ungroup()

# Plot data as heatmap
heatmap_trans <- ggplot(trans_all_day, aes(x= datestamp, y = location, fill = transactions, text = paste("category: ", category))) +
              theme_minimal() +
              geom_tile(colour="white") +
              labs(x = "Date", y = "Location") +
              scale_fill_gradient(low = "#dfe1eb", high = "#261fb4") +
              scale_x_date(breaks = "1 week")
              labs(title = "Daily Transactions",
                   x = "Date", fill = "Transactions")
              
ggplotly(heatmap_trans)

```

#### Q1 Anomaly - Credit Card Posting at Fixed Hour
We observed that some locations only have credit card transactions posted in 1 hour within the day e.g. Bean There Done That and Brewed Awakenings, Jack's Magical Beans, Coffee Shack at 12PM.
Corrections: Not required
```{r credit card postings, echo=FALSE, warning=FALSE, include=TRUE}
# Aggregate transactions by hour
trans_all_hour <- trans_all %>%
                 left_join(location_labels_POI, by = "location") %>%
                 mutate(hour = as.character.numeric_version(get_hour(timestamp))) %>%
                 dplyr::select(category, hour, location) %>% 
                 group_by(category, hour, location) %>%
                 summarise(transactions = n()) %>%
                 ungroup()

# Plot hourly data into a heatmap
heatmap_trans_hour <- ggplot(trans_all_hour, aes(x= hour, y = location, fill = transactions, text = paste("category: ", category))) +
              theme_minimal() +
              geom_tile(colour="white") +
              labs(x = "Hour", y = "Location") +
              scale_fill_gradient(low = "#dfe1eb", high = "#261fb4") +
              #scale_x_datetime(breaks = "6 hours")
              labs(title = "Hourly Transactions by Locations",
                   x = "Hour", fill = "Transactions")
              
ggplotly(heatmap_trans_hour)

```

#### Q1 Anomaly - Kronos Mart Postings are 1 day late
Kronos Mart's credit card postings usually come 1 day late, which result in strange timing of the postings at 3-9AM. 
Corrections: The date of the loyalty card transaction should replace the credit card timestamp. 
```{r late postings, echo=FALSE, warning=FALSE, include=TRUE}
kronos_delay <- cc_delay_post_2 %>% filter(location == "Kronos Mart") %>%
                    mutate(cc = as_date(timestamp.x)) %>%
                    dplyr::select(price, location, cc, timestamp.y) %>% 
                    rename(loyalty = timestamp.y) %>%
                    pivot_longer(!c(location,price), names_to = "type", values_to = "date") %>%
        
# Plot scatterplot of Kronos Mart delayed postings
p <- ggplot(kronos_delay, aes(x=date, y = price, color = type, label = price)) +
  geom_point() +
  geom_text(check_overlap = TRUE, size = 2, nudge_x = 0.5) +
  theme_minimal() +
  labs(title = "Daily Transactions of Kronos Mart by Card Type",
                   x = "Date", y = "Price ($)", color = "Card Type")

ggplotly(p)
```

#### Q1 Anomaly - Cashbacks in Multiples of $20
We observe that some loyalty card transactions have prices lower than the credit card transactions tied to it. The denominations is usually in multiples of $20, ranging from $20 to $80. Locations vary widely which we guess could be a loyalty cashback scheme. 
Corrections: We will use the credit card amount as loyalty price is non-consequential to investigation.
```{r cashback, echo=FALSE, warning=FALSE, include=TRUE}
# 219 transactions match with price range with price difference in increments of $20.
cashback_p <- loyalty_cashback %>% 
      dplyr::select(location.x, price_diff) %>%
      rename(location =  location.x)


p <- ggplot(cashback_p, aes(x = price_diff, fill = location)) + 
  geom_histogram(boundary = 1, position = "stack") +
  labs(title = "Distribution of Cashback by Locations",
                   x = "Cashback Amount ($)", y = "Count", fill = "Locations")
  theme_minimal()
  
ggplotly(p)  
```

#### Q1 Anomaly - Multiple Credit Cards to Loyalty Cards
We found unexpected relationships: 1286 & L3288, L6267 - 6691 & 6899
Corrections: Care was taken to join credit card and loyalty card transactions to prevent duplication.
```{r multiple match, echo=FALSE, warning=FALSE, include=TRUE}
visNetwork(nodes, edges, width = "100%", main = list(text = "Relationship Between Credit Card and Loyalty Cards",
 style = "font-family:arial;font-size:20px;")) %>% 
  visOptions(highlightNearest = TRUE, nodesIdSelection = list(enabled = TRUE,
                                                              selected = "74")) %>% 
  visGroups(groupname = "cc", color = "lightblue") %>%    
  visGroups(groupname = "loyalty", color = "orange") %>% 
  visLegend(width = 0.1, position = "right", main = list(text = "Card Types",
 style = "font-family:arial;font-size:15px;"))
```

#### Q1 Anomaly - High Spend Due to Possible Fraud
The highest spend on the credit card is $10k on Frydos Autospply n' More that does not have a loyalty card transaction, belonging to credit card 9551. 9 credit cards register many high spends that are at industrial areas tying to the 9 truck drivers. The other highest spend that is not in industrial location is on Albert's Fine Clothing by credit card 1321. 

Using the data table, we noticed that there is a gap between the the $10k spend on 13 Jan and the next purchase by credit card 9551 on 16 Jan. This suggests that the high spend might be due to fraud. 
```{r high spends, echo=FALSE, warning=FALSE, include=TRUE}
d <- highlight_key(trans_all)
spend_cc <-  ggplot(d, aes(x = last4ccnum, y = price, color = location)) +
            geom_point() + theme_minimal() + theme(axis.text.x=element_text(angle=45, hjust=1))

gg_cc <- highlight(ggplotly(spend_cc), "plotly_selected")

crosstalk::bscols(gg_cc, DT::datatable(d), widths = 10)

```

```{r high spends, echo=FALSE, warning=FALSE, include=TRUE}
d <- highlight_key(trans_all)
spend_cc <-  ggplot(d, aes(x = last4ccnum, y = price, color = location)) +
            geom_point() + theme_minimal() + theme(axis.text.x=element_text(angle=45, hjust=1))

gg_cc <- highlight(ggplotly(spend_cc), "plotly_selected")

crosstalk::bscols(gg_cc, DT::datatable(d), widths = 10)

```

#### Q1 Anomaly - Sparse Spends
xx. 
```{r high spends, echo=FALSE, warning=FALSE, include=TRUE}
ly_count <- trans_all %>%
  group_by(datestamp, last4ccnum) %>%
  summarise(n = n()) %>% 
  drop_na() %>% ungroup()

ly_count_p <- ggplot(ly_count, aes(x = datestamp, y = n)) + 
  geom_bar(stat='identity') +
  facet_wrap(~last4ccnum) 

ggplotly(ly_count_p)
```



### Question 2
Add the vehicle data to your analysis of the credit and loyalty card data. How does your assessment of the anomalies in question 1 change based on this new data? What discrepancies between vehicle, credit, and loyalty card data do you find? Please limit your answer to 8 images and 500 words.

::: l-body
|No| Commentary          | Visual                                       |
|--|---------------------|----------------------------------------------|
|1| Confirm credit card fraud | ![](./images/Q3_1.png){#id .class width=100%}|
|2| Company spend by truck drivers who work in shifts | ![](./images/Q3_2.png){#id .class width=100%}|
|3| Spatial contradictions between credit card and car locations | ![](./images/Q3_3.png){#id .class width=100%}|
|4| No car movement on weekend mornings | ![](./images/Q3_4.png){#id .class width=100%}|
|5| text| ![](./images/Q3_5.png){#id .class width=100%}|
|6| text| ![](./images/Q3_6.png){#id .class width=100%}|
|7| text| ![](./images/Q3_7.png){#id .class width=100%}|
|8| text| ![](./images/Q3_8.png){#id .class width=100%}|
:::

#### Q2 Anomaly - High Spend due to Fraud
The highest spend on the credit card is on Frydos Autospply n' More that does not have a loyalty card transaction, belonging to credit card 9551. 9 credit cards register many high spends that are at industrial areas such as the Airport, chemical plant, refinery, fabrication, scrapyard. The other highest spend that is not industrial is on Albert's Fine Clothing by credit card 1321. 
```{r high spends, echo=FALSE, warning=FALSE, include=TRUE}
```

#### Q2 Discrepancies - Different locations registered on transactions vs gps 
xx
```{r xx, echo=FALSE, warning=FALSE, include=TRUE}
```



### Question 3
Can you infer the owners of each credit card and loyalty card? What is your evidence? Where are there uncertainties in your method? Where are there uncertainties in the data? Please limit your answer to 8 images and 500 words.

::: l-body
|No| Commentary          | Visual                                       |
|--|---------------------|----------------------------------------------|
|1| Owners of credit card | ![](./images/Q3_1.png){#id .class width=100%}|
|2|  | ![](./images/Q3_2.png){#id .class width=100%}|
|3| Uncertainty | ![](./images/Q3_3.png){#id .class width=100%}|
|4| text| ![](./images/Q3_4.png){#id .class width=100%}|
|5| text| ![](./images/Q3_5.png){#id .class width=100%}|
|6| text| ![](./images/Q3_6.png){#id .class width=100%}|
|7| text| ![](./images/Q3_7.png){#id .class width=100%}|
|8| text| ![](./images/Q3_8.png){#id .class width=100%}|
:::

### Question 4
Given the data sources provided, identify potential informal or unofficial relationships among GASTech personnel. Provide evidence for these relationships. Please limit your response to 8 images and 500 words.

::: l-body
|No| Commentary          | Visual                                       |
|--|---------------------|----------------------------------------------|
|1| Elsa(7) and Brand(33) seeing each other | ![](./images/Q4_1.png){#id .class width=100%}|
|2| Inga(13), Isia(16), Loreto(15), Hennie(21), Minke(24) conducting suspicious activities | ![](./images/Q4_2.png){#id .class width=100%}|
|3| Kanon(22) and Borrasca(28) hang out| ![](./images/Q4_3.png){#id .class width=100%}|
|4| Shared homes of employees | ![](./images/Q4_4.png){#id .class width=100%}|
|5| Dinner party among engineers and IT | ![](./images/Q4_5.png){#id .class width=100%}|
|6| text| ![](./images/Q4_6.png){#id .class width=100%}|
|7| text| ![](./images/Q4_7.png){#id .class width=100%}|
|8| text| ![](./images/Q4_8.png){#id .class width=100%}|
:::

### Question 5
Do you see evidence of suspicious activity? Identify 1- 10 locations where you believe the suspicious activity is occurring, and why. Please limit your response to 10 images and 500 words.

::: l-body
|No| Commentary          | Visual                                       |
|--|---------------------|----------------------------------------------|
|1| 5 suspicious locations | ![](./images/Q5_1.png){#id .class width=100%}|
|2| Surveillance on 4 executive houses | ![](./images/Q5_2.png){#id .class width=100%}|
|3| Hennie's unusual home stays | ![](./images/Q5_3.png){#id .class width=100%}|
|4| Friday dinner party at Lars' home | ![](./images/Q5_4.png){#id .class width=100%}|
|5| Golf gathering with CEO | ![](./images/Q5_5.png){#id .class width=100%}|
|6| Nils overtime work| ![](./images/Q5_6.png){#id .class width=100%}|
|7| Gustav's spotty GPS | ![](./images/Q5_7.png){#id .class width=100%}|
|8| Mid-day rendezvous at Chostus Hotel| ![](./images/Q5_8.png){#id .class width=100%}|
|9| Inactivity of trucks | ![](./images/Q5_9.png){#id .class width=100%}|
|10| Bertrand's late night trip | ![](./images/Q5_10.png){#id .class width=100%}|
:::


